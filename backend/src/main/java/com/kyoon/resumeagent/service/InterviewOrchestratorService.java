...
public class InterviewOrchestratorService {
    ...
    public JsonNode analyzeAndSave(Long assessmentId, String itemName, String qnaJson, String extraCapCode, boolean skipDelete) throws Exception {
        ...
        var prompt = new PromptTemplate(resourceLoader.getResource("classpath:prompts/DataExtractor.st"))
                .create(Map.of("jobGroup", jobGroup, "itemName", itemName, "qna", filteredQnaJson));
    ...
    private String filterEmptyAnswers(String qnaJson) {
        try {
            JsonNode arr = objectMapper.readTree(qnaJson);
            if (!arr.isArray()) return qnaJson;

            List<JsonNode> filtered = new ArrayList<>();
            arr.forEach(qa -> {
                String answer = qa.has("answer") ? qa.get("answer").asText("").trim() : "";
                if (!answer.isEmpty()) filtered.add(qa);
            });
